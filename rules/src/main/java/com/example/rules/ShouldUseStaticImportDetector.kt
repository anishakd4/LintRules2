...
class ShouldUseStaticImportDetector : Detector(), Detector.UastScanner {
    ...
    override fun visitMethodCall(context: JavaContext, node: UCallExpression, method: PsiMethod) {
        ...
        if (methodName != null && className != null) {
            ...
            if (!isStaticallyImported && matches) {
                context.report(ISSUE, node, context.getNameLocation(node), "Anish: Should statically import $methodName")
            }
        }
    }
        ...
        if (processed.add(reference)) {
            ...
            if (matches && isQualified) {
                context.report(
                    ...
                    "Anish: Should statically import ${UastLintUtils.getQualifiedName(referenced)}"
                )
            }
        }
    }
    ...
    companion object {
        ...
        val ISSUE = Issue.create(
            ...
            PRIORITY,
            Severity.FATAL,
            Implementation(ShouldUseStaticImportDetector::class.java, EnumSet.of(Scope.JAVA_FILE))
        )
    }
}